...
bool LEDs_Init(void)
{
 ...
 // PORTA_PCR11 |= PORT_PCR_DSE_MASK; // Sets all of the port a registers to 1
...
{
  //GPIOA_PCOR |= GPIO_PCOR_PTCO(color);  // sets the colour of the led to the set register
  GPIOA_PCOR = (color);
}
...
{
  //GPIOA_PSOR  |= GPIO_PCOR_PTCO(color);  // led turns off writes to clear register
  GPIOA_PSOR = (color);
}
...
{
 // GPIOA_PTOR |= GPIO_PTOR_PTTO(color);  // change the colour of the led
  GPIOA_PTOR =(color);
}
